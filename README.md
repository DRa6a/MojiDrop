...
- **AI 问答**：输入 `#q: 你的问题 `（以空格结尾）后，模组会向 AI 请求简短回答，并在建议列表显示约 10 字的答案概况，按 TAB 或点击即可将完整答案（带 `a:` 前缀）填入聊天框。
...
5. 输入 `#q: 你的问题 `（注意结尾空格）可向 AI 提问，返回后在建议栏看到答案概况，按 `TAB` 填充完整答案。
...
```
You are a kaomoji suggestion assistant. Based on the chat context provided, suggest 1 to %d relevant kaomoji (Japanese emoticons). Return only the kaomoji, either one per line or separated by commas. Do not include explanations, labels, or markdown formatting.
```
...
```
You are a concise Q&A assistant. Answer the user's question briefly and clearly. Keep answers short.
```

建议保留“简短回答”相关描述，避免 AI 返回过长内容导致请求变慢。
